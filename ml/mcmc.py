...
#ref:http://users-deprecated.aims.ac.za/~ioana/cp5.pdf
...
def density(x, mu, sigma):
	...
	x_minus_mu = x - mu
	return exp(-0.5 * dot(dot(transpose(x_minus_mu), inv_sigma), x_minus_mu)) / (2 * pi * sqrt(linalg.det(sigma)))

mu = array([0,0])
sigma = array([[4,1], [1, 4]])

print density(0, mu, sigma)
